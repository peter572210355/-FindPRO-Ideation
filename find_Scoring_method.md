...
在推薦系統的情境中，常用「Top-K」來表示系統輸出的前 K 名候選人（例如 Top-10 代表系統推薦的前 10 名）。相關的評估指標（如 Precision@K、HitRate@K、NDCG@K）都是針對這前 K 名來衡量系統的準確度與實用性。
...
計算結果：
- 命中數：$h_i(5) = 2$（因為 A、B 都被命中）
- Precision@5 = $2/5 = 0.4$
- HitRate@5 = 1（有命中至少一個）
- MAP：  
  - Precision@3 = 1/3 = 0.33（因 A 在第 3 位命中第 1 個）  
  - Precision@5 = 2/5 = 0.4（因 B 在第 5 位命中第 2 個）  
  - AP = (0.33 + 0.4)/2 = 0.365 
---
