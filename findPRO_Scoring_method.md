...
1. **候選池覆蓋率（召回問題）**  
   - 召回 = 從「全部學者資料庫」中，先快速抓一批「可能相關的人」出來，形成一個候選池（例如前 500 名）。  
   - 覆蓋率 = 在這批候選人裡，有沒有包含「真正該出現的審查委員」。  
   - 如果在召回階段就漏掉了正確的人，那後續排序再好也找不到。  
   - 👉 白話：像辦比賽找裁判，先從全國隨便抓 500 個「看起來跟體育有關的人」出來。如果真正的足球裁判壓根沒被抓進來，就算後面再排序也沒用。

2. **Top-K 命中率 / Success@K（排序問題）**  
   - 排序 = 在候選池中，決定誰排在最前面。  
   - 命中率 / Success@K = 看在前 K 個位置裡，有沒有把正確的人排進去。  
   - 即使召回有抓到正確的人，但如果他永遠被排在第 200 名，使用者也看不到 → 排序就失敗了。  
   - 👉 白話：剛剛抓進來的 500 個人裡面，真的有足球裁判。但如果他被排在第 200 名，使用者只看前 20 名，還是看不到。
